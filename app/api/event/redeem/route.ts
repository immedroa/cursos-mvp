...
export async function POST(request: Request) {
  try {
    ...
    const { eventId, pointsSpent } = await request.json()

    if (!eventId) {
      return NextResponse.json({ error: 'eventId es requerido' }, { status: 400 })
    }

    // 1. Insertar el registro de canje en la tabla existente mentor_redemptions
    // Usamos el prefijo 'event:' en mentor_id para distinguir
    const { data: redemption, error: redemptionError } = await supabase
      .from('mentor_redemptions')
      .insert({
        user_id: user.id,
        mentor_id: `event:${eventId}`,
    ...
    if (redemptionError || !redemption) {
      console.error('Error al registrar canje de evento:', redemptionError)
    ...
    console.error('Error en API route event redemption:', error)
